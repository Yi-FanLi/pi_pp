...
dists_array = np.zeros([nsamp, int(nO*(nO-1)/2)])
...
for iloop in range(nloop):
  ...
  t3 = time()
  if rank==0:
    print("Loop %d: computing pbc distances costs %4.f s."%(iloop, t3-t1))
  for ibatch in range(nbatch):
    isamp = iloop*nbatch + ibatch
    dists_array[isamp] = dist_r[ibatch][np.triu_indices(nO, 1)]
  ...
  #print(hist_r[0].sum())
  #print(r_array)
    g_r_array[isamp] = 2*hist_r[0]/4/np.pi/r_array**2/dr/nO/(nO-1)*Vol
    if rank==0:
      if (isamp+1)%100==0:
        t4 = time()
...
if rank==0:
  ...
  #print(g_r.shape)
  np.save(directory+"goo.npy", np.c_[r_array, g_r])
  #plt.xlim(2, 7.5)
